...
Ongoing work by the PIs indicates the viability of this structure is highly metric dependent.  Consider the Hamming metric over $\mathcal{M}^\ell$ defined as $d(a,b) = | \{ 1\le i\le \ell | a_i \neq b_i\}|$.  Let $f:[\ell] \rightarrow [\ell]$ be a permutation on the integers $1,..., \ell$, let $g_i: \mathcal{M}\rightarrow \mathcal{M}$.  Let $x\in\mathcal{M}^\ell$ be a plaintext points.  A distance-preserving encryption scheme $\enc: \mathcal{M}^\ell\rightarrow \mathcal{M}^\ell$ must be of the form $\enc(x) = f(g_1(x_1),..., g_\ell(x_\ell))$.  Intuitively the permutation $f$ rotates the dimensions will the permutations $g_i$ shuffle individual dimensions.  These are the only operations that preserve distance across the entire dataset.
 ...
 \paragraph{Extensions} The PIs propose to extend this work to other metrics as well, in particular Euclidean, cosine-similarity, and Jaccard distance.    The PIs also plan to address the case of \emph{course grained} distance, e.g., where distance is in the set $\{far, equal, near\}$.  This has interesting connections to property-preserving encryption for \emph{graph data}, since a course metric can be represented by a graph where neighboring vertices are near according to the metric.  In other words, we are interested as encrypting a graph as another graph which preserves the structure.  This corresponds to \emph{automorphisms} of the graph (permutations of the vertices that preserve the structure) and is well-studied.
