...
Ongoing work by the PIs indicates the viability of this structure is highly metric dependent.  Consider the Hamming metric over $\mathcal{M}^\ell$ defined as $d(a,b) = \{ 1\le i\le \ell | a_i \neq b_i\}$.  Let $f:[\ell] \rightarrow [\ell]$ be a permutation on the integers $1,..., \ell$, let $g_i: \mathcal{M}\rightarrow \mathcal{M}$ and let $mask\in \mathcal{M}^\ell$.  Let $x\in\mathcal{M}^\ell$ be a plaintext points.  A distance-preserving encryption scheme $\enc: \mathcal{M}^\ell\rightarrow \mathcal{M}^\ell$ must be of the form $\enc(x) = f(g_1(x_1),..., g_\ell(x_\ell)) \oplus mask$.  For polynomial size $\mathcal{M}$ an adversary can completely learn $\enc(x)$ with a polynomial number of plaintext/ciphertext pairs.  Prior work of the PI only provided security when $\mathcal{M}$ was superpolynomial in size~\cite{EPRINT:ABCFG16}, which we now know was necessary.

\paragraph{Tasks}
\begin{enumerate}
\item Understand the viability of distance-preserving encryption for other metrics.
\item How strong can the ideal object be for distance-revealing encryption.
\item Constructions based on fuzzy extractors.
\end{enumerate}
